...
const repCortoCLI = {
  ...
  reporteCortoCitiCLI: async (req, res) => {
    // console.log(req);
    const campaign = 1013;
    const nameProcedure = "sps_reporte_corto";
    let parameters = { option: 5 };
    ...
    if (objConnOracle == null || objConnOracle == undefined) {
      ...
      try {
        ...
        if (oraRspta == 0) {
          ...
          const worksheet = workbook.addWorksheet('Reporte_Corto_CLI');

          const ws_data = [
              [fecha_actual, "CORTE", "", "CLI", ""],
...
async function aplicarEstilos(ws_data, worksheet, workbook, res) {
  ...
  const outputPath = path.join(__dirname, "..", 'assets/docs', 'Reporte_Corto_CLI.xlsx');
  ...
  // const filePath = path.join(__dirname, "..", "assets/docs", "Reporte_Corto_CLI.xlsx");
  try {
  // // Escribir el archivo
  ...
  // console.log(`Archivo guardado en ${filePath}`);
    ...
    if (mails.length == 0) {
      ...
      const mailOptions = {
          to: emailString,                
          subject: 'Reporte Corto Citi CLI',      
          html: 'Reporte Corto Citi CLI',
          attachments: [
          {
              filename: 'Reporte_Corto_CLI.xlsx',
              path: filePath 
          }
          ]
      }
...
//c7d7ca
  ...
  const filePath_ = path.join(__dirname, "..", "assets/docs", "Reporte_Corto_CLI.xlsx");
  ...
  const mails= await actionsMysql.mailsAsistencia(4);
    if (mails.length == 0) {
      ...
      if (nObjMailer == 200) {
          console.log('Reporte de CLI enviado: ', nObjMailer)
          await actionsMysql.saveLogCron("REPORTE DE CLI",1);
                  
          res.json({ status: 200, rspta: "MAILS SENT" });

      }else {
          await actionsMysql.saveLogCron("REPORTE DE CLI",0);
          console.log('Error al enviar reporte de CLI: ', nObjMailer)
          res.send('Error al enviar reporte de CLI')
      } 
    }
}


module.exports = { repCortoCLI };
