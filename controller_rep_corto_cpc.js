...
const repCortoCPC = {
  ...
  reporteCortoCitiCPC: async (req, res) => {
    // console.log(req);
    const campaign = 1013;
    const nameProcedure = "sps_reporte_corto";
    let parameters = { option: 2 };
    ...
    if (objConnOracle == null || objConnOracle == undefined) {
      ...
      try {
        ...
        if (oraRspta == 0) {
          ...
          const worksheet = workbook.addWorksheet('Reporte_Corto_CPC');

          const ws_data = [
              [fecha_actual, "CORTE", "", "CPC", ""],
...
async function aplicarEstilos(ws_data, worksheet, workbook, res) {
  ...
  const outputPath = path.join(__dirname, "..", 'assets/docs', 'Reporte_Corto_CPC.xlsx');
  ...
  // const filePath = path.join(__dirname, "..", "assets/docs", "Reporte_Corto_CPC.xlsx");
  try {
  // // Escribir el archivo
...
/*
    const mails= await actionsMysql.mailsAsistencia(3);
    if (mails.length == 0) {
      res.json({ status: 200, rspta: "Reporte de asistencia no enviado, sin remitentes para este correo" });
    } else {
      ...
      const mailOptions = {
          to: emailString,                
          subject: 'Reporte Corto Citi CPC',      
          html: 'Reporte Corto Citi CPC',
          attachments: [
          {
              filename: 'Reporte_Corto_CPC.xlsx',
              path: filePath 
          }
          ]
      }
...
//c7d7ca
  ...
  const filePath_ = path.join(__dirname, "..", "assets/docs", "Reporte_Corto_CPC.xlsx");
  ...
  const mails= await actionsMysql.mailsAsistencia(3);
    if (mails.length == 0) {
      ...
      if (nObjMailer == 200) {
          console.log('Reporte de CPC enviado: ', nObjMailer)
          await actionsMysql.saveLogCron("REPORTE DE CPC",1);
                  
          res.json({ status: 200, rspta: "MAILS SENT" });

      }else {
          await actionsMysql.saveLogCron("REPORTE DE CPC",0);
          console.log('Error al enviar reporte de CPC: ', nObjMailer)
          res.send('Error al enviar reporte de CPC')
      } 
    }
}


module.exports = { repCortoCPC };
